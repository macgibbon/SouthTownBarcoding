...
public class MainController {
	...
	private void handleUPCEmbedded() throws WriterException {
		...
		String payload = weightStr; // length should be 11
	...
	private void handleGS1() throws WriterException {
		...
		String weightLbStr = productCodeField.getText();
		if (weightLbStr == null || weightLbStr.isBlank()) {
			messageLabel.setText("Enter a weight in kilograms (e.g. 1.234).");
			return;
		}
		weightLbStr = weightLbStr.trim();
		BigDecimal weightLb;
		try {
			weightLb = new BigDecimal(weightLbStr);
			if (weightLb.signum() < 0) {
		...
		// inputs)
		BigDecimal scaled = weightLb.movePointRight(3).setScale(0, RoundingMode.HALF_UP);
		 long weightInt;
		 try {
		...
		/*
		 * // AI 310n requires 6-digit numeric field (leading zeros) if (weightInt < 0
		 * || weightInt > 999999L) { messageLabel.
		 * setText("Scaled weight must be between 0 and 999999 for AI 310n."); return; }
		 * String weightField = String.format("%06d", weightInt);
		 * 
		 * // Build GS1-128 payload: // FNC1 prefix + (01)GTIN + (310n)weight // For
		 * encoding we use the FNC1 character '\u00f1' as the initial char ZXing
		 * recognizes. // The content passed to ZXing: FNC1 + "01" + gtin14 + "310" +
		 * decimals + weight6 String fnc1 = "\u00f1"; String ai01 = "01"; String
		 * ai310prefix = "310" + decimals; // e.g., 3103 String content = fnc1 + ai01 +
		 * gtin + ai310prefix + weightField;
		 * 
		 ...
		 * messageLabel.setText("GS1-128 generated (AI01 + AI310" + decimals +
		 * "): GTIN=" + gtin + " weightScaled=" + weightField);
		 */
	}
